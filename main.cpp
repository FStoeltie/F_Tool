...
void removeTemplate(string& file)	{
	string t_identifier("++--++");
	std::size_t eot = file.find_last_of(t_identifier); // end of template
...
void validateTemplateVersion(string& template_content, string& file)	{
	...
	if(file_template_tag == std::string::npos)	{
		return;
	}
	...
	std::size_t template_version_tag = template_t_version.find(version_identifier);
	if(template_version_tag == std::string::npos)	{
		return;
	}
	
	string t_template_content = template_content;
	t_template_content.erase(0, file_template_tag + version_identifier.length())
	stringstream ss1(t_template_content);
	int template_version;
	ss1 >> template_version;
